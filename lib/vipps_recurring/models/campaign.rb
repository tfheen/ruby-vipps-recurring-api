...
module VippsRecurring
  class Campaign
    ...
    # The date and time the campaign starts. Needs to be UTC
    attr_accessor :start
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `VippsRecurring::Campaign` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `VippsRecurring::Campaign`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @campaign_price.nil?
        invalid_properties.push('invalid value for "campaign_price", campaign_price cannot be nil.')
      end

      if @campaign_price < 100
        invalid_properties.push('invalid value for "campaign_price", must be greater than or equal to 100.')
      end
    ...
    def valid?
      return false if @campaign_price.nil?
      return false if @campaign_price < 100
    ...
    # @param [Object] campaign_price Value to be assigned
    def campaign_price=(campaign_price)
      if campaign_price.nil?
        fail ArgumentError, 'campaign_price cannot be nil'
      end

      if campaign_price < 100
        fail ArgumentError, 'invalid value for "campaign_price", must be greater than or equal to 100.'
      end
    ...
    def hash
      [campaign_price, _end, start].hash
    end
